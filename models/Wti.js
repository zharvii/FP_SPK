...
async function hargaTerbaru() {
  const data = await investing("commodities/crude-oil", "P1D", "PT1M");
...
async function hargaPerjam() {
  const data = await investing("commodities/crude-oil", "P1D", "PT1H");
  return data;
}

async function hargaPerhari() {
  const data = await investing("commodities/crude-oil", "P1M", "PT5H");
  return data;
}

async function ramal() {
  const data = await investing("commodities/crude-oil", "P1Y", "P1M");
  let harga = [];
  data.forEach((obj) => {
